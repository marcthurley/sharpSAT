...
private:
  ...
  void exploreClause(VariableIndex var, ClauseIndex clID, LiteralID * pstart_cls){
	  auto itVEnd = search_stack_.end();
	  bool all_lits_active = true;
	  unsigned x = var_frequency_scores_[var];
	  bool appeared = false;
	  assert(!archetype_.clause_nil(clID));
	  ...
	  assert(x <= var_frequency_scores_[var]);
	  if (!archetype_.clause_nil(clID)){


		 // assert(appeared);
		  var_frequency_scores_[var]++;
		        	if(x + 1!= var_frequency_scores_[var])
		        		cout << var_frequency_scores_[var] - x << " ";

...
void ComponentCache::init() {
  ...
  if (config_.maximum_cache_size_bytes == 0
      || info.totalram < config_.maximum_cache_size_bytes)
    config_.maximum_cache_size_bytes = info.totalram;

  cout << "Maximum cache size:\t" << config_.maximum_cache_size_bytes / 1000000
      << " MB" << endl << endl;
...
CacheEntryID ComponentCache::createEntryFor(Component &comp,
  ...
  if (statistics_.cache_bytes_memory_usage_
      ...
      if (info.freeram < min_free_ram_) {
        cout << endl << endl << "LOW FREE RAM, deleting cache entries" << endl<< endl;
        deleteEntries();
        sysinfo(&info);
        if (info.freeram < min_free_ram_) {
          cout << endl << endl << "LOW FREE RAM, cannot store entry" << endl<< endl;
          return false;
        }
      }
    }
...
unsigned long ComponentCache::recompute_bytes_memory_usage() {
  ...
  for (auto pbucket : table_)
    if (pbucket != nullptr)
      statistics_.cache_bytes_memory_usage_ += pbucket->getBytesMemoryUsage();
...
bool ComponentCache::requestValueOf(Component &comp, mpz_class &rn) {
  ...
  for (auto it = table_[v]->begin(); it != table_[v]->end(); it++) {
    pcomp = &entry(*it);
    if (packedcomp.hashkey() == pcomp->hashkey() && pcomp->equals(packedcomp)) {
  ...
  for (auto vt = super_comp.varsBegin(); *vt != varsSENTINEL; vt++)
    if (variables_seen_[*vt] == CA_IN_SUP_COMP) {
      ...
      if (component_search_stack_.size() == 1) {
        ...
        component_stack_.back()->reserveSpace(component_search_stack_.size(),
